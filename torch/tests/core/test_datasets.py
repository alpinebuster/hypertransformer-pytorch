...
class DatasetsTest(tf.test.TestCase):
    ...
    def _make_data(self, batch_size=4, image_size=4):
        ...
        ds = tf.data.Dataset.from_tensor_slices(
            {
                "image": np.zeros(
                    shape=(batch_size, image_size, image_size, 1), dtype=np.float32
                ),
        ...
        with self.session() as sess:
            return datasets.make_numpy_data(
                sess, ds, batch_size=1, num_labels=4, samples_per_label=repetitions
            )
    ...
    def test_get_batch(self):
        ...
        images, labels, classes = gen.get_batch(
            batch_size=batch_size, config=aug_config
        )
        with self.session() as sess:
            sess.run(tf.global_variables_initializer())
