...
class MultiHeadAttention(tf.Module):
    ...
    def _split_heads(self, x, batch_size, depth):
...
class DecoderLayer(tf.Module):
    ...
    def __init__(self, params, name=None):
        ...
        if params.internal_dim > 0:
            self.ffn = PWFeedForward(
                dim=params.mha_output_dim, internal_dim=params.internal_dim, name="fc"
            )
    ...
    def __call__(
        ...
        with self.name_scope, tf.variable_scope(None, default_name=self.name):
            ...
            attn_2, attn_weights_block_2 = self.mha_2(
                enc_output, enc_output, out_1, padding_mask
            )
...
class Decoder(tf.Module):
    ...
    def __call__(
        self, x, enc_output, is_training, look_ahead_mask=None, padding_mask=None
    ):
        with self.name_scope, tf.variable_scope(None, default_name=self.name):
            for i in range(self.num_layers):
