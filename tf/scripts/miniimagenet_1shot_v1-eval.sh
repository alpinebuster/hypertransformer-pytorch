# Call `./scripts/miniimagenet_1shot_v1-eval.sh` from the `tf` root
# 
# e.g. `nohup ./scripts/miniimagenet_1shot_v1-eval.sh --data_dir=../miniimagenet --data_numpy_dir=../miniimagenet/cache --train_log_dir=../miniimagenet/logs > miniimagenet-eval.log 2>&1 &`
# DS: `--train_dataset=miniimagenet` # or imagenette, emnist

python -m hypertransformer.evaluate --num_layerwise_features=16 --default_num_channels=16 \
